...
export default function App() {
  ...
  if (showOverview && !reviewMode) {
    return (
      <div className="min-h-screen bg-gray-50 font-sans text-gray-900 flex items-center justify-center px-6 py-10">
        <div className="w-full max-w-3xl bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
          <div className="px-8 py-6 border-b border-gray-200">
            <p className="text-[11px] uppercase tracking-wide text-gray-500">Operations Assessment</p>
            <h1 className="text-lg font-semibold text-gray-900 mt-1">6-Month Self-Assessment Overview</h1>
          </div>
          ...
          <div className="px-8 py-7">
            ...
            <ul className="mt-4 space-y-1.5 text-sm text-gray-700 leading-relaxed list-disc pl-5">
            ...
            <div className="mt-7 flex justify-end">
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2 rounded text-xs font-medium bg-gray-900 text-white hover:bg-gray-800 transition-colors"
              >
                Begin Assessment
  ...
  if (reviewMode) {
    if (!claimsLoaded) {
      return (
        <div className="h-screen w-full flex flex-col items-center justify-center bg-gray-50">
          <Loader2 className="w-8 h-8 animate-spin mb-4 text-gray-600" />
          <p className="text-sm font-medium text-gray-600">Checking reviewer access...</p>
        </div>
      );
    }
      ...
      return (
        <div className="h-screen w-full flex flex-col items-center justify-center bg-gray-50 px-4">
          <div className="max-w-md w-full bg-white border border-gray-200 rounded-xl shadow-sm p-6 text-center">
            ...
            <p className="mt-2 text-sm text-gray-600">You do not have permission to view reviewed submissions.</p>
            <button
              onClick={returnToAssessmentOverview}
              className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-md bg-gray-800 text-white text-sm font-medium hover:bg-gray-900 transition-colors"
            >
              Back to Assessment
            </button>
          </div>
    ...
    return (
      <div className="min-h-screen bg-gray-50 font-sans text-gray-900">
        <header className="sticky top-0 z-10 bg-gray-50/95 backdrop-blur-sm border-b border-gray-200/60">
          <div className="max-w-7xl mx-auto px-5 md:px-8 py-3 flex items-center justify-between">
            <div>
              <h1 className="text-sm md:text-base font-semibold text-gray-900">Assessment Reviewer</h1>
              <p className="text-[11px] text-gray-500">Completed submissions for review and analysis</p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={returnToAssessmentOverview}
                className="flex items-center gap-1.5 text-[12px] font-medium px-3 py-1.5 rounded-md bg-gray-800 border border-gray-800 text-white hover:bg-gray-900 transition-colors shadow-sm"
              >
                Back to Assessment
              </button>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-5 md:px-8 py-6 grid grid-cols-1 lg:grid-cols-[340px,1fr] gap-6">
          <aside className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden h-fit">
            <div className="px-4 py-3 border-b border-gray-100">
              <p className="text-xs font-semibold text-gray-800">Submissions</p>
              <p className="text-[11px] text-gray-400">Latest 50 records</p>
            </div>
            <div className="max-h-[70vh] overflow-y-auto p-2 space-y-2">
              ...
              {!reviewLoading && reviewSubmissions.length === 0 && !reviewError && (
                <div className="px-3 py-2 rounded-md bg-gray-50 border border-gray-100 text-[12px] text-gray-500">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className={`text-[11px] font-medium truncate ${isActive ? 'text-gray-100' : 'text-gray-500'}`}>
                        {submission.userId || 'Unknown user'}
                      </span>
                      <span className={`text-[10px] font-semibold ${isActive ? 'text-gray-300' : 'text-gray-700'}`}>
                        {submission.totals?.completionPercent ?? 0}%
                      </span>
                    </div>
                    <p className={`mt-1 text-[11px] ${isActive ? 'text-gray-300' : 'text-gray-400'}`}>
          ...
          <main className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
            {!selectedSubmission ? (
                ...
                <div className="px-6 py-4 border-b border-gray-100">
                  ...
                  <div className="mt-2 grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div className="bg-gray-50 border border-gray-100 rounded-lg px-3 py-2">
                      ...
                      <p className="text-[12px] font-medium text-gray-800 break-all">{selectedSubmission.userId || 'Unknown'}</p>
                    </div>
                    <div className="bg-gray-50 border border-gray-100 rounded-lg px-3 py-2">
                      <p className="text-[10px] uppercase tracking-wide text-gray-400">Submitted</p>
                      <p className="text-[12px] font-medium text-gray-800">{selectedSubmission.submittedAtDate ? selectedSubmission.submittedAtDate.toLocaleString() : 'No timestamp'}</p>
                    </div>
                    <div className="bg-gray-50 border border-gray-100 rounded-lg px-3 py-2">
                      <p className="text-[10px] uppercase tracking-wide text-gray-400">Completion</p>
                      <p className="text-[12px] font-medium text-gray-800">{selectedSubmission.totals?.answered ?? 0} / {selectedSubmission.totals?.questions ?? 0}</p>
                    </div>
                  </div>
                </div>
                  ...
                  {selectedSubmissionSections.map((section) => (
                    <section key={section.id} className="space-y-3">
                      ...
                      {section.subsections.map((subsection) => (
                        <div key={subsection.id} className="border border-gray-100 rounded-lg overflow-hidden">
                          <div className="px-4 py-2.5 bg-gray-50 border-b border-gray-100">
                            <p className="text-[12px] font-medium text-gray-700">{subsection.title}</p>
                          </div>
                            ...
                            {subsection.questions.map((question) => {
                              ...
                              return (
                                <div key={question.id} className="px-4 py-3">
                                  <p className="text-[12px] font-medium text-gray-800">{question.label}</p>
                                  <p className="mt-2 text-[12px] leading-relaxed text-gray-600 whitespace-pre-wrap">
                                    {hasValue ? String(value) : 'No response provided.'}
                                  </p>
                                  {meta && (
                                    <p className="mt-1 text-[10px] text-gray-400">{meta.sectionTitle} • {meta.subsectionTitle}</p>
                                  )}
                                </div>
                              );
                            })}
    ...
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900 flex flex-col md:flex-row">
          ...
          <div className="max-w-3xl mx-auto px-5 md:px-10 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex items-center gap-1.5 text-[12px] font-medium px-3 py-1.5 rounded-md bg-white border border-gray-200 text-gray-700 hover:bg-gray-100 transition-colors shadow-sm"
              >
                Overview
              </button>
              {isReviewer && (
              ...
              return (
                <div key={subsection.id} className="space-y-4">
                  ...
                  <div className="flex items-start gap-3">
                    <span className="text-sm font-bold text-gray-700 tabular-nums shrink-0 mt-0.5">
                      {subIdx + 1}.
                    </span>
          ...
          <div className="mt-10 flex items-center justify-between pb-12">
            ...
            {activeSection < SECTIONS.length - 1 ? (
              ...
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-semibold bg-gray-900 text-white hover:bg-black disabled:opacity-40 transition-colors shadow-sm"
              >
      ...
      {user && (
        <button
          ...
          className="fixed bottom-3 right-3 bg-gray-900 text-white text-[10px] px-3 py-1.5 rounded-md flex items-center gap-2 opacity-30 hover:opacity-100 transition-opacity"
        >
          <div className="w-1.5 h-1.5 bg-gray-300 rounded-full" />
          <span className="text-gray-400 break-all max-w-[220px] text-left">{user.uid}</span>
          <span className="text-gray-500">•</span>
          <span className="text-gray-400">{isReviewer ? 'Reviewer' : 'Standard'}</span>
        </button>
      )}
    </div>
  );
